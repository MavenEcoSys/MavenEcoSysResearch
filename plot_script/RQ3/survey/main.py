...
def aware_of_cve():
    ...
    plt.legend(loc='lower center',ncol=4, bbox_to_anchor=(0.5,-0.68), )
...
def update_reason():
    ...
    colors=["orange","#009e79","#AAAAAA","#AAAAAA","#333333"]
    hatches=["xxxx","","xxxx","",""]
    edgecolors=["white","none","white","none","none"]
    fontcolor=["black","black","black","black","white"]
    not_upgrade = [18, 9, 4]
    reason_list = ["not maintained", "not affected", "others"]
    
    colors1=["orange","#009e79","#333333","#AAAAAA"] 
    hatches1=["","xxxx","","",""]
    edgecolors1=["white","white","none","none","none"]
    fontcolor1=["black","black","white","black","white"]

    for i in range(len(not_upgrade)):
        axes[0].bar_label(axes[0].barh("No Response", not_upgrade[i], align="center", left=sum(not_upgrade[:i]), height=0.45, label=reason_list[i],color=colors1[i],hatch=hatches1[i],edgecolor=edgecolors1[i]), label_type="center", fontsize=16,color=fontcolor1[i])

    reason_list = ["security issue", "normal upgrade", "get rid of warning", "compatibility issues", "others"]
    upgrade = [30, 10, 8, 2, 1]
    for i in range(len(upgrade)):
        axes[1].bar_label(axes[1].barh("Response", upgrade[i], align="center", left=sum(upgrade[:i]), height=0.45, label=reason_list[i],color=colors[i],hatch=hatches[i],edgecolor=edgecolors[i]), label_type="center", fontsize=16,color=fontcolor[i])
    ...
    axes[1].legend(bbox_to_anchor=(1.0, -0.6), ncol=3, )
...
def how_find_affected():
    ...
    not_upgrade = [13, 1]
    for i in range(len(not_upgrade)):
        axes[0].bar_label(axes[0].barh("No Response", not_upgrade[i], align="center", left=sum(not_upgrade[:i]), height=0.1, label=reason_list[i],color=colors[i],hatch=hatches[i]), label_type="center",fontsize=16)
    plt.sca(axes[0])
    plt.yticks(fontsize=16)
    plt.sca(axes[1])
    plt.yticks(fontsize=16)
    ...
    for i in range(len(upgrade)):
        axes[1].bar_label(axes[1].barh("With Response", upgrade[i], align="center", left=sum(upgrade[:i]), height=0.1, label=reason_list[i],color=colors[i],hatch=hatches[i],edgecolor=edgecolors[i]), label_type="center", fontsize=16, color=fontcolor[i])
    axes[1].legend(loc='lower center',ncol=2, bbox_to_anchor=(0.4,-1.7), )
